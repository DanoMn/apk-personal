...
Fuente canónica: `meta/instructions/2026-06-02-onboarding-introduccion-diseno.md` §2.8, §7 · `docs/sueno/decisiones-diseno-sueno-v1.md` §9 (D1) · `openspec/specs/onboarding-sleep/spec.md` (consentimiento wind-down)
Cambio origen: `onboarding-introduccion` (slice 5 — último slice)
...
forma perezosa. Consume el consentimiento capturado en el Bloque Sueño (`windDownConsentGiven`)
...
- GIVEN el slice de notificaciones está aplicado
...
- Ejemplos válidos: "No detectamos datos de sueño en los últimos 3 días. Puedes revisar
  el permiso de uso en la configuración."
...
- GIVEN el onboarding-notifications aplicado
- WHEN se inspecciona el esquema Room y las migraciones
- THEN no existe entidad ni migración nueva introducida únicamente por este slice
...
- Configuración de las notificaciones dentro de la app (más allá del consentimiento
  wind-down ya capturado en onboarding-sleep); la desactivación se hace desde el sistema Android.
...
| D-APAGA | ¿La Notif B se apaga sola al detectar que el permiso UsageStats fue concedido? | No especificado — si hay datos, el contador se resetea naturalmente | Aclarar si se quiere suprimir proactivamente cuando UsageStats está activo |
...
- **Runtime (`verificacion-por-capas.md`, capas 1-4 aplicables):**
  ...
  - Sin datos de sueño 3 días → la Notificación B aparece.
