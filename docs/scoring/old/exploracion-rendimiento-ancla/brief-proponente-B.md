# Brief — Proponente B: Acoplamiento suave
...
**Tu sesgo asignado — acoplamiento suave:** partís de la familia de funciones donde días y tiempo se
COMBINAN (multiplicación, promedios generalizados, interpolaciones) de forma equilibrada, con la
frecuencia ganando por poco. Fórmulas más balanceadas que las de dominancia extrema.
**Atención crítica:** la decisión de producto "constancia > intensidad" es estructural — tu
acoplamiento NO puede permitir que ráfagas de tiempo en pocos días le ganen a la consistencia en días
(caso testigo: F=5, T=20, D=2, t_i=[60,60] debe quedar claramente dominado por el déficit de
frecuencia). Si tu balance lo permite, tu propuesta falla.
...
- Decisión cerrada del producto: la app premia CONSTANCIA (días) por encima de ráfagas de
  productividad de 1 día (tiempo). Tu acoplamiento balanceado debe respetarla estructuralmente.
- El motor viejo del código usaba `0.70·días + 0.30·tiempo` con gates duros (`UNBREAKABLE_*`,
  worst-term). Está descartado por diseño: NO lo uses como punto de partida ni como referencia — en
  particular, un promedio ponderado lineal fijo días/tiempo es exactamente lo que falló. Si tu familia
  termina en algo lineal, tenés que demostrar por qué el tuyo no repite esos defectos.
- Tu salida alimenta el promedio de la capa: un ancla en `[0, 1+]` se promedia con sus hermanas.

## 10. Entregable

Escribí **un solo archivo**: `docs/scoring/exploracion-rendimiento-ancla/subagente-B-propuesta.md`

Secciones requeridas (en este orden):
1. **Sesgo asignado** — cómo interpretaste el acoplamiento suave.
