# Brief — Proponente A: Dominancia fuerte de frecuencia
...
**Tu sesgo asignado — dominancia fuerte de frecuencia:** partís de la familia de funciones donde los
días pesan tanto que el tiempo casi no mueve la aguja salvo en extremos. `D/F` manda; el tiempo es un
ajuste menor. Explorá hasta dónde esa dominancia puede llevarse sin romper los axiomas.
**Atención crítica:** en `F = 7` no existe superávit de días posible — tu fórmula DEBE seguir
permitiendo que el superávit de tiempo, por sí solo, alcance Inquebrantable (problema P2). Si tu
dominancia es tan absoluta que el tiempo queda incapaz de mover el estado, tu propuesta falla.
...
acerca a 7. Todo relativo a la configuración del usuario. Esto acota la dominancia de frecuencia.
...
- Decisión cerrada del producto: la app premia CONSTANCIA (días) por encima de ráfagas de
  productividad de 1 día (tiempo). Tu sesgo es la expresión más fuerte de esa decisión — pero P2 la acota.
...
Escribí **un solo archivo**: `docs/scoring/exploracion-rendimiento-ancla/subagente-A-propuesta.md`

Secciones requeridas (en este orden):
1. **Sesgo asignado** — cómo interpretaste la dominancia fuerte de frecuencia.
